...
module ActiveRecord
  module TimeScope
    class TimeProxy < AbstractProxy
      ...
      def before(time)        ; where "#{attr} < ?", time.in_time_zone       ; end
      ...
      def after(time)        ; where "#{attr} > ?", time.in_time_zone ; end
      ...
      def at_or_before(time)        ; where "#{attr} <= ?", time.in_time_zone      ; end
      ...
      def at_or_after(time)        ; where "#{attr} >= ?", time.in_time_zone           ; end
